...
class NetIpv4(db.Model):
    ...
    subnet_ipv4 = db.Column(db.Integer(), nullable=False)
    netmask = db.Column(db.Integer(), nullable=False)
    default_gw = db.Column(db.Integer(), nullable=False)
    broadcast = db.Column(db.Integer(), nullable=False)
    ip_range_start = db.Column(db.Integer(), nullable=False)
    ip_range_end = db.Column(db.Integer(), nullable=False)
    failover_peer = db.Column(db.VARCHAR(20), nullable=False, server_default='nr-dhcpd-failover')
...
class HostsAllow(db.Model):
    # список хостов, которые получают настройки сы dhcp сервера
    id = db.Column(db.SmallInteger(), primary_key=True, autoincrement=True)
...
class ReservedIpv4(db.Model):
    ...
    hostname = db.Column(db.VARCHAR(32), nullable=False)
    mac_addr = db.Column(db.VARCHAR(18), nullable=False, unique=True)
    reserved_ipv4 = db.Column(db.Integer(), nullable=False, unique=True)
